...
async def bot_respond(
    ...
    if not history:
        return history, build_reasoning_html([])

    last_user = next((m["content"] for m in reversed(history) if m["role"] == "user"), "")
    conversation_str = _history_to_string(history)

    turn_events: List[EventRecord] = []

    def _listener(rec: EventRecord) -> None:
        turn_events.append(rec)

    unsubscribe = event_bus.subscribe(_listener)
    try:
        result = await handle_turn(last_user, conversation_str, wrapper)
    finally:
        unsubscribe()
    ...
    reasoning_html = build_reasoning_html(turn_events)
    return new_history, reasoning_html


def clear_chat(
    wrapper: RunContextWrapper[ConversationState],
) -> tuple[List[Dict[str, str]], str, RunContextWrapper[ConversationState]]:
    wrapper.context = _new_conversation_state()
    return [], build_reasoning_html([]), wrapper


def build_app() -> gr.Blocks:
    with gr.Blocks(css=CUSTOM_CSS, title="Agentic Reminder Assistant") as demo:
        ...
        clear_btn.click(
            clear_chat,
            [wrapper_state],
            [chatbot, reasoning_html, wrapper_state],
        )
